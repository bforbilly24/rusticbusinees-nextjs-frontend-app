	...
	return (
		<div className='team-area md:py-155 py-[55px] border-[#dfdfdf] border-b'>
			<div className='container'>
				<h2 className='lm:text-[42px] text-[32px] lm:leading-[50px] mb-[60px]'>Profressional & Passionate</h2>
				<div className='grid sm:grid-cols-12 lg:gap-x-[70px] fixed-md:gap-x-[30px] gap-[30px]'>
					{teamItems?.map((teamItem) => {
						...
						return (
							<div className='md:col-span-4 fixed-lm:col-span-6 fixed-sm:col-span-6' key={teamItem.id}>
								<div className='team-member overflow-hidden group'>
									<div className={teamMemberImage}>
										<Link href='/about'>
											<a>
												<Image src={teamItem?.image} alt={teamItem?.title} width={343} height={409} layout='responsive' objectFit='cover' quality={70} priority />
											</a>
										</Link>
									...
									<div className='team-member-content pt-[25px]'>
										<h3>
											<Link href='/about'>
												<a className='transition-all hover:text-[#caac6b] text-[24px] leading-[31px]'>{teamItem?.memberName}</a>
											</Link>
										</h3>
										<span className='designation text-secondary leading-6 block text-[14px] mt-[5px]'>{teamItem?.designation}</span>
									</div>
								</div>
							</div>
						);
