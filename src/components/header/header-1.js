...
function HeaderOne() {
	...
	const isSticky = (e) => {
		...
		scrollTop >= 90 ? header.current?.classList.add('is-sticky') : header.current?.classList.remove('is-sticky');
	};
	//   End Here
								...
								<div className={`outer-box ${headerCss}`}>
									...
									<div className='offcanvas-area'>
										<div className='offcanvas'>
											<button type='button' className='menu-bars flex text-white text-[24px]' aria-label='Right Align'>
					...
					<nav className='offcanvas-menu' onClick={(e) => e.stopPropagation()}>
						<ul className='offcanvas-menu-items'>
							<li className='navbar-toggle flex justify-between items-center pb-[15px]'>
								...
								<button type='button' className='menu-bars text-[24px] opacity-80 hover:opacity-50 transition-all' aria-label='Right Align'>
